...
namespace SQLAPI_Consumer
{
    ...
    public static class APIConsumer
    {
            ...
            catch (WebException ex)
            {
                using (var stream = ex.Response?.GetResponseStream())
                {
                    if (stream != null)
                    {
                        ...
                        {
                            responseStream = new System.IO.Compression.GZipStream(stream, CompressionMode.Decompress);
                        }
        ...
        /// Request GraphQL
        /// </summary>
        /// <param name="url">GraphQL url</param>
        /// <param name="query">GraphQL query</param>
        /// <param name="variablesJson">variables for write mutation</param>
        /// <param name="JsonHeaders">Json header</param>
        /// <returns>Json Response</returns>
        public static string GraphQL(string url, string query, string variablesJson, string JsonHeaders = "")
        {
            ...
            try
            {
                ...
                {
                    query = query,
                    variables = string.IsNullOrEmpty(variablesJson)
                        ? null
                        : JsonConvert.DeserializeObject(variablesJson)
                };
                ...
                {
                    ContentResult = streamReader.ReadToEnd();
                }
